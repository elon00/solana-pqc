 ...
 * - Solana Dual Hybrid Transaction Conjunction (Ed25519 ∧ ML-DSA-65)
 */
...
try {
  ...
  console.log('\n▶ [TIER 2] Canonical Hash & Solana PDA Invariants:');
  ...
  const pdaPayload = Buffer.from('Solana Quantum Vault PDA Commitment');
  const pdaCommitment = Buffer.from(sha256(pdaPayload)).toString('hex');
  assert.strictEqual(pdaCommitment.length, 64);
  pass('Solana Quantum PDA commitment produces exact 32 bytes hex');

  console.log('\n▶ [TIER 3] NIST FIPS 203 ML-KEM-768 Lattice Execution:');
  const seed64 = new Uint8Array(64).fill(0x42);
  ...
  const seed32 = new Uint8Array(32).fill(0x71);
  ...
  const tamperedMsg = Buffer.from('Solana Quantum Vault Transfer Authorization Invariant!');
  ...
  console.log('\n▶ [TIER 7] Solana Dual Hybrid Conjunction Conformance:');
  const { publicKey: edPub, privateKey: edPriv } = crypto.generateKeyPairSync('ed25519');
  const edSig = crypto.sign(null, msg, edPriv);
  const edOk = crypto.verify(null, msg, edPub, edSig);
  assert.strictEqual(edOk, true);
  pass('Solana Ed25519 signature verified cleanly');

  const dualHybridOk = edOk && verified;
  assert.strictEqual(dualHybridOk, true);
  pass('Solana dual conjunction holds when both Ed25519 and ML-DSA are valid');

  const failClosedOk = edOk && badSigVer; // badSigVer is false
  assert.strictEqual(failClosedOk, false);
  pass('Solana dual conjunction fails-closed when PQC component is compromised');
